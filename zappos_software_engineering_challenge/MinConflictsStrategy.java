...
public class MinConflictsStrategy extends SolutionStrategy {
    ...
    private Assignment generateRandomAssignment(CSP csp) {
        ...
        csp.getVariables().stream().forEach((var) -> {
            Object randomValue = Util.selectRandomlyFromList
                (csp.getDomain(var).asList());
            assignment.setAssignment(var, randomValue);
        });
        return assignment;
    }

    private List<Variable> getConflictedVariables
        (Assignment assignment, CSP csp) {
        List<Variable> result = new ArrayList<>();
        csp.getConstraints().stream().filter((constraint)
                -> (!constraint.isSatisfiedWith(assignment))).forEach
                    ((constraint) -> {
                    constraint.getScope().stream().filter((var)
                            -> (!result.contains(var))).forEach((var)
                            -> {
                        result.add(var);
                    });
                });
    ...
    private Object getMinConflictValueFor
        (Variable var, Assignment assignment, CSP csp) {
    ...
    private int countConflicts(Assignment assignment,
        ...
        int result = 0;
        result = constraints.stream().filter((constraint)
                -> (!constraint.isSatisfiedWith(assignment))).map
                    ((_item) -> 1).reduce(result, Integer::sum);
        return result;
    }
}
